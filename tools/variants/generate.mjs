...
function loadDatabase() {
  ...
  for (const device of data.devices) {
    ...
    if (device.target !== "mcs51") {
      throw new Error(`invalid compiler target ${device.target} for ${device.model}`);
    }
    if (device.cpp_core_profile !== "stc-cxx11-12mhz-experimental" ||
        device.default_clock_hz !== 12000000 ||
        JSON.stringify(device.clock_options_hz) !== "[12000000]" ||
        device.default_memory_model !== "large" ||
        JSON.stringify(device.memory_models) !== '["large"]' ||
        device.idata_bytes !== 256 || device.edata_bytes !== 0 ||
        device.linker !== undefined || device.mcs251_linker !== undefined ||
        device.maximum_code_bytes > 65536) {
      throw new Error(`unsupported MCS51 C++ build profile for ${device.model}`);
    }
...
function variantName(device) {
  return device.model.replaceAll("-", "_").replaceAll("+", "_PLUS");
}
...
function renderBoards(devices) {
  const lines = ['# Generated by tools/variants/generate.mjs; edit devices.json instead.', 'menu.clock=CPU clock (must match ISP configuration)', ''];
  for (const d of devices) {
    const values = {
      name: d.model, 'build.core':'STC', 'build.board':d.macro, 'build.mcu_macro':d.macro,
      'build.variant':variantName(d), 'build.f_cpu':d.default_clock_hz+'L', 'build.memory_model':'large',
      'build.target':'-mmcs51', 'build.mode_flags':'-DSTC_EXECUTION_MODE_MCS51',
      'build.core_flags':renderCoreFlags(d), 'build.timer_flags':renderTimerFlags(d),
    ...
    for (const [k,v] of Object.entries(values)) lines.push(d.id+'.'+k+'='+v);
    for (const hz of d.clock_options_hz) {lines.push(d.id+'.menu.clock.'+clockId(hz)+'='+clockLabel(hz));lines.push(d.id+'.menu.clock.'+clockId(hz)+'.build.f_cpu='+hz+'L');}
    lines.push('');
  }
  return lines.join('\n')+'\n';
}
...
function renderMetadata(database, device) {
  ...
  const metadata = {
    ...
    experimental_compiler_target: true,
...
try {
  ...
  const expected = new Set(['_common', ...database.devices.map(variantName)]);
  if (existsSync(variantRoot)) for (const entry of readdirSync(variantRoot, {withFileTypes:true})) {
    if (entry.isDirectory() && !expected.has(entry.name)) throw new Error('orphan variant: '+entry.name);
  }
