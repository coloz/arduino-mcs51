...
pub fn adapt(
    ...
    c = sort_typedefs(&c)?;
    c = helpers(&c)?;
    // MCS51 vtables contain tagged generic pointers, but indirect calls use
    // 16-bit program addresses. Discard the tag through uintptr_t explicitly.
    c = normalize_program_pointers(&c)?;
    ...
    // CBE relies on native headers for sqrtf/powf and other declarations.
    for header in ["#include <math.h>", "#include <string.h>"] {
        if prefix.lines().any(|line| line == header) {
            preamble.push_str(header);
            preamble.push('\n');
        }
    }
    if c.contains("llvm_cbe_program_pointer") {
        ensure!(prefix.contains("typedef void (*llvm_cbe_program_pointer)(void);"),
                "missing CBE program pointer definition");
        preamble.push_str("typedef void (*llvm_cbe_program_pointer)(void);\n");
    }
...
fn constructors(ir: &str) -> Result<Vec<(u32, String)>> {
    ...
    // MCS51's program address space is P1. LLVM uses it for constructor
    // entries too, even though ordinary data pointers live in address space 0.
    let pointer = r"ptr(?:\s+addrspace\(1\))?";
    let re = Regex::new(&format!(
        r#"\{{\s*i32,\s*{pointer},\s*{pointer}\s*\}}\s*\{{\s*i32\s+(\d+),\s*{pointer}\s+@(?:"([^"]+)"|([A-Za-z0-9_.$-]+)),\s*{pointer}\s+null\s*\}}"#,
    ))?;
...
pub(crate) fn audit_ir(ir: &str, roots: &BTreeSet<String>) -> Result<()> {
    ...
    let masked = Regex::new(r#"c"(?:\\.|[^"\\])*"|;[^\n]*"#)?.replace_all(ir, "");
    for pattern in [
        r"\baddrspace\s*\(\s*(?:[2-9][0-9]*|1[0-9]+)\s*\)",
...
fn normalize_program_pointers(c: &str) -> Result<String> {
    let re = Regex::new(r"\(llvm_cbe_program_pointer\)(_[0-9]+)")?;
    let masked = mask(c);
    let mut result = c.to_owned();
    for row in re.captures_iter(&masked).collect::<Vec<_>>().into_iter().rev() {
        result.replace_range(row.get(0).unwrap().range(),
            &format!("(llvm_cbe_program_pointer)(uintptr_t){}", &row[1]));
    }
    Ok(result)
}
...
pub fn audit_warnings(log: &str, c: &str) -> Result<()> {
    ...
    for line in log.lines().filter(|l| l.to_lowercase().contains("warning")) {
        ...
        let accepted = code == 84
            ...
            || code == 151 && verified_vtable_warning(line, &m[2], c)?;
        ensure!(accepted, "unqualified bridge compiler warning: {line}");
    }
    Ok(())
}
fn verified_vtable_warning(line: &str, message: &str, c: &str) -> Result<bool> {
    if !Regex::new(r"^using generic pointer \S+ to initialize \S+$")?.is_match(message) {
        return Ok(false);
    }
    let location = Regex::new(r":(\d+):\s*warning 151:")?;
    let Some(row) = location.captures(line) else { return Ok(false) };
    let number: usize = row[1].parse()?;
    let source = c.lines().nth(number.saturating_sub(1)).unwrap_or("");
    Ok(Regex::new(r"^static const struct \w+ _ZTV\w+\s*=")?.is_match(source))
}
...
mod tests {
    use super::*;
    #[test]
    fn mcs51_program_pointers_drop_tags_only_in_code() {
        let source = "call((llvm_cbe_program_pointer)_4); /* (llvm_cbe_program_pointer)_5 */\nconst char *s = \"(llvm_cbe_program_pointer)_6\";";
        let adapted = normalize_program_pointers(source).unwrap();
        assert_eq!(adapted, source.replacen("(llvm_cbe_program_pointer)_4", "(llvm_cbe_program_pointer)(uintptr_t)_4", 1));
        assert_eq!(normalize_program_pointers(&adapted).unwrap(), adapted);
        assert!(audit_ir("define void @f() addrspace(1) {\nret void\n}", &BTreeSet::new()).is_ok());
        assert!(audit_ir("define void @f() addrspace(10) {\nret void\n}", &BTreeSet::new()).is_err());
    }
    #[test]
    fn mcs51_vtable_warning_requires_matching_source_location() {
        let source = "static const struct table _ZTV4Test = {0};\nvoid *other = 0;\n";
        let message = "using generic pointer _function to initialize _ZTV4Test";
        assert!(verified_vtable_warning("adapted.c:1: warning 151:", message, source).unwrap());
        assert!(!verified_vtable_warning("adapted.c:2: warning 151:", message, source).unwrap());
        assert!(!verified_vtable_warning("adapted.c:1: warning 151:", "unrelated diagnostic", source).unwrap());
    }
    #[test]
    ...
    fn constructor_shape() {
        ...
        let program = ir.replace("ptr", "ptr addrspace(1)");
        assert_eq!(constructors(&program).unwrap(), constructors(ir).unwrap());
        assert!(constructors(&program.replace("addrspace(1)", "addrspace(2)")).is_err());
        assert!(constructors(&program.replace("[1 x", "[2 x")).is_err());
        assert!(constructors(&program.replace("null", "@associated")).is_err());
    }
}
