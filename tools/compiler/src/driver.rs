...
impl Driver {
    ...
    fn configured(
        ...
        let mut d = Self {
            ...
            arguments,
            frontend,
            assembler: exe(&sdcc_root.join("bin"), "sdas8051"),
            includes: sdcc_root.join("include"),
            runtime: sdcc_root.join("lib/large-stack-auto"),
    ...
    fn parse(&mut self) -> Result<()> {
        ...
        while let Some(a) = args.next() {
            ...
            if a == "-DSTCXX_LINK_OPT" || a.starts_with("-DSTCXX_LINK_OPT=") {
                ...
                !["-mmcs251", "-DSTCXX_TARGET_MCS251=1", "-DSTC16F40K128"].contains(&a.as_str())
                    && !a.starts_with("-DSTC_EXECUTION_MODE_MCS251"),
                "This platform requires the MCS51 target"
            );
        ...
        ensure!(cpp && target, "MCS51 C++ profile is required");
        ensure!(
            clock_count == 1 && self.clock == "12000000",
        ...
        for flag in [
            ...
            "-DSTCXX_TARGET_ENDIAN_LITTLE=1",
            "-DSTCXX_TARGET_ENDIAN_BIG=0",
        ] {
            self.clang_args.push(flag.into());
            self.sdcc_args.push(flag.into());
        }
        ensure!(self.stack.is_empty() && !self.constrained, "MCS251 stack/heap flags are invalid on MCS51");
        Ok(())
    }
    ...
    pub fn verify_with(&self, verifier: &mut HashVerifier) -> Result<()> {
        ...
        for name in ["sdcc", "sdar", "sdas8051", "sdld", "sdcpp"] {
            verifier.check(
        ...
        for (name, key) in [
            ...
            ("mcs51.lib", "target_runtime_sha256"),
        ] {
            verifier.check(
                self.runtime.join(name),
                string(&self.lock["targets"]["mcs51"]["sdcc_inputs"][key])?,
            )?;
        }
        Ok(())
    }
