...
fn check_heap(heap: &Path, state: &Path, constrained: bool) -> Result<()> {
    ...
    for name in ["___sdcc_heap", "___sdcc_heap_size", "___stcxx_heap_init"] {
        ensure!(defs.contains(name), "heap missing {name}");
    }
    ensure!(!defs.contains("___sdcc_heap_size32"), "wrong heap ABI");
...
fn verify_heap_map(_d: &Driver, map: &str, heap: &Path) -> Result<()> {
    let heap_symbols = ["___sdcc_heap", "___sdcc_heap_size", "___stcxx_heap_init"];
