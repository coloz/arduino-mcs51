...
fn entry(mut args: Vec<String>) -> Result<()> {
    ...
    if !["--platform", "compile", "archive", "link"].contains(&args[0].as_str()) {
        bail!("This MCS51 driver supports Arduino recipes only; use the separate STCXX SDK for standalone builds");
    }
